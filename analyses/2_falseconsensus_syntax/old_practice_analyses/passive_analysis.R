...
props = transformed %>%
  ...
  summarize(nYes = sum(yes), 
             ...
             # meanAgreeCILow = ci.low(meanAgreementEstimate), 
             # meanAgreeCIHigh = ci.high(meanAgreementEstimate), 
             # entropyCILow = ci.low(entropy), 
             # entropyCIHigh = ci.high(entropy)) %>%

meanAgreeCILow = ci.low(props$meanAgreementEstimate) 
meanAgreeCIHigh = ci.high(props$meanAgreementEstimate) 
entropyCILow = ci.low(props$entropy) 
entropyCIHigh = ci.high(props$entropy) 

props = props %>% 
  mutate(entropy = case_when(is.nan(entropy) ~ 0,
                              TRUE ~ entropy),
          entropyYMin = entropy - entropyCILow,
          entropyYMax = entropy + entropyCIHigh,
          meanAgreeYMin = meanAgreementEstimate - meanAgreeCILow,
          meanAgreeYMax = meanAgreementEstimate + meanAgreeCIHigh) %>% 
  data.frame() %>% 
  mutate(version = relevel(as.factor(version), ref = "uncovered"), 
         version = as.numeric(version))


# entropyYMin = props$entropy - entropyCILow
# entropyYMax = props$entropy + entropyCIHigh
# meanAgreeYMin = (props$meanAgreementEstimate) - meanAgreeCILow
# meanAgreeYMax = (props$meanAgreementEstimate) + meanAgreeCIHigh
...
cpassive = myCenter(props$passive)

# cpassive = myCenter(data.frame(props[,c("passive")]))

centered_m1 <- cbind(props, cpassive)

# m1 <- brm(entropy ~ cpassive * version + (1 + (cpassive * version)|item),
#           data = centered_m1)

model <- lmer(entropy ~ cpassive * version + (1 + (cpassive * version)|item),
         data = props)
summary(model)

model2 <- lm(entropy ~ cpassive * version,
              data = props)
summary(model2)
####
...
cpassive = myCenter(transformed$passive)
centered_m2 <- cbind(transformed, cpassive)
cindividual_judgment = myCenter(transformed$individual_judgment)
centered_m2 <- cbind(transformed, cindividual_judgment)
ctrue_proportion = myCenter(transformed$true_proportion)
centered_m2 <- cbind(transformed, ctrue_proportion)


centropy = myCenter(props$entropy)
centered_m5<-cbind(props, centropy)
centered_m5<cbind(props, transformed)


model5 <- lm(population_judgement ~ cEntropy * cPassive)

# lm model for q2 dep variable 


model3 <- lm(population_judgment ~ cpassive *cindividual_judgment *ctrue_proportion, data = centered_m2)
summary(model3)


model3b <- lm(population_judgment ~ cpassive *cindividual_judgment *ctrue_proportion + (1 + (cpassive * cindividual_judgment * ctrue_proportion)|item) + (1 + (cpassive  * cindividual_judgment * ctrue_proportion)|workerid), data = centered_m2)
summary(model3)
# 
...
escape2 <- props %>% 
  ...
  filter(center_embedding == "no") %>%  #filter to certain items, then only if center_embedding = No 
  select(item, syntax_condition, version, meanAgreementEstimate, entropy, entropyYMin, entropyYMax, meanAgreeYMin, meanAgreeYMax)%>% 
  unique() %>% 
  mutate(syntax_condition = fct_relevel(syntax_condition, "NCEA", "NCEP", "CEA", "CEP"))
# props <- escape %>%
#   filter(version %in% c("uncovered","covered", "controversial")) %>% 
...
#plot 2: Entropy VS. SYNTAX FOR passive items 
...
ggplot(data = escape2,
  ...
  ylab("Entropy") +
  xlab("Difficult-to-Process Language Presence") + 
  ggtitle("Effect of DPL on Entropy for Controversial Passive Items")

#################################################################################

#plot 3: MAE VS. SYNTAX FOR passive items
...
ggplot(data = escape2,
  ...
  xlab("Difficult-to-Process Language Presence") + 
  ggtitle("Effect of DPL on Mean Agreement Estimate for Controversial Passive Items")

#################################################################################

#plot 3: MAE VS. ENtropy FOR passive items 
ggplot(data = escape2,
